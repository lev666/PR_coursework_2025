...
int find_word_sep(char *str, int *w_len, int *sep_len) {
    ...
    *sep_len = 0;
    while (*temp_ptr != '\0' && *temp_ptr != '.' &&
           !(isalnum(*temp_ptr) || *temp_ptr == '-')) {
        (*sep_len)++;
        temp_ptr++;
    }

    if (*sep_len == 0 && *w_len == 0)
        return 0;

    return 1;
}

err_code split_sent(strs_all *strs) {
    ...
    for (size_t i = 0; i < strs->total_len; i += 2) {
        ...
        char *ptr2 =
            (i + 1 < strs->total_len) ? strs->str_inform[i + 1].str : NULL;
        ...
        while ((ptr1 && *ptr1 != '\0') || (ptr2 && *ptr2 != '\0')) {
            ...
            int dot1 = 0, dot2 = 0;
            if (ptr1[w1_len + sep1_str_len] == '.')
                dot1 = 1;
            if ((has_w2) && (ptr2[w2_len + sep2_str_len] == '.'))
                dot2 = 1;

            char *curr_sep = (words_ind % 2 == 0) ? sep1 : sep2;

            if (has_w1)
                printf("%.*s", w1_len, ptr1);
            else
                printf("%s", filler);
            ...
            if (has_w2)
                printf("%.*s", w2_len, ptr2);
            else
                printf("%s", filler);

            if (sep1_str)
                printf("%.*s", sep1_str_len, sep1_str);
            if (sep2_str)
                printf("%.*s", sep2_str_len, sep2_str);
            ...
            if (has_w1)
                ptr1 += w1_len + sep1_str_len + dot1;
            if (has_w2)
                ptr2 += w2_len + sep2_str_len + dot2;
